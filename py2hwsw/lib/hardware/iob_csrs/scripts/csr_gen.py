...
class csr_gen:
    ...
    def gen_wr_reg(self, row):
        ...
        if isinstance(addr, int) and isinstance(addr_w, int):
            lines += f"    assign {name}_addressed = (addr>>shift_amount >= ({addr}>>shift_amount)) && (addr>>shift_amount < iob_min(1,{addr+2**addr_w}>>shift_amount));\n"
        else:
            lines += f"    assign {name}_addressed = (addr>>shift_amount >= ({addr}>>shift_amount)) && (addr>>shift_amount < iob_min(({addr}+(2**({addr_w})))>>shift_amount));\n"
        ...
        if auto:  # generate register
            ...
            wires.append(
                {
                    ...
                    "descr": "",
                    "signals": [
                        {"name": f"{name}_wvalid", "width": 1},
                    ],
                },
            )
            lines += f"    assign {name}_wvalid = internal_iob_valid & (write_en & {name}_addressed);\n"
            lines += "    iob_reg_cae #(\n"
            ...
            if suffix:
                ...
                # lines += f"    assign {name}_ready{suffix_input} = {name}_ready;\n" # FIXME:
    ...
    def gen_rd_reg(self, row):
        ...
        if not auto:  # output read enable
            ...
            if (
                "W" not in row.mode
            ):  # Use same valid as generated by "W" (no need for new one)
                lines += f"    assign {name}_valid{suffix} = internal_iob_valid & {name}_addressed_r;\n"
            lines += (
        ...
        for row in table:
            ...
            if "W" in row.mode:
                if auto:
                    ...
                    # NOTE: Write iob signals: valid addr wdata wstrb ready
                    register_signals += []
                    port_has_outputs = True
                    # FIXME: Find out how to suppress linter warnings
                    snippet += f"""
   assign {name}_addr{"" if row.internal_use else "_o"} = internal_iob_addr_stable[ADDR_W-1:2]-{addr>>2};
"""
                    register_signals += [
                        {
            ...
            if "R" in row.mode:
                if auto:
                    ...
                    # NOTE: Read iob signals: valid addr rdata ready rready rvalid
                    # FIXME: Find out how to suppress linter warnings
                    snippet += f"""
   assign {name}_addr{"" if row.internal_use else "_o"} = internal_iob_addr_stable[ADDR_W-1:2]-{addr>>2};
"""
...
"""
        ...
        for row in table:
            if "W" in row.mode:
                ...
                snippet += "    wire [ADDR_W-1:0] addr;\n"
                snippet += "    assign addr = `IOB_WORD_ADDR(internal_iob_addr_stable) + byte_offset;\n"
                snippet += """
...
"""
        ...
        for row in table:
            ...
            if "W" in row.mode:
                if not auto:
                    # get ready
                    snippet += f"        if((addr >= {addr}) && (addr < {addr + 2**addr_w})) begin\n"
                    snippet += (
