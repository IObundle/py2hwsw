...
#define MODER_DEFAULT (MODER_RXEN | MODER_TXEN | MODER_CRC | MODER_PAD | MODER_FULLD)
...
static int send_cmd_internal(int fd, uint8_t cmd, const void *data, uint16_t len,
    ...
    errno = 0;
    if (sendto(fd, &pkt, HDR_SIZE + (data ? len : 0), 0,
...
static int get_stats(struct net_stats *stats) {
    ...
    sscanf(p, "%lu %lu %lu %lu %lu %lu %lu %lu "
           ...
           &stats->tx_dropped, &stats->tx_fifo_errors, &stats->tx_collisions,
...
static int set_iface_flags(int fd, unsigned int flags_to_set, unsigned int flags_to_clear) {
    struct ifreq ifr;
    int ret;
...
static int test_interface_detection(void) {
    ...
    snprintf(driver_path, sizeof(driver_path),
             "/sys/class/net/%s/device/driver", g_iface);
...
static int test_interface_up(void) {
    ...
    if (set_iface_flags(fd, IFF_UP | IFF_RUNNING, 0) < 0) {
        TEST_FAIL("Interface Bring-Up",
                  "Failed to bring interface up: %s", strerror(errno));
        close(fd);
        return -1;
    }
...
static int test_default_regs(void) {
    ...
    if (get_ethtool_regs(fd, regs, &count) < 0) {
        if (errno == EOPNOTSUPP) {
            TEST_INFO("Default Registers", "ethtool get_regs not supported by driver");
            close(fd);
    ...
    if (count < REG_ETH_END / 4) {
        TEST_FAIL("Default Registers",
                  "Register dump too short: %d words (expected >= %d)",
                  count, REG_ETH_END / 4);
        return -1;
    }
    ...
    if ((moder & MODER_DEFAULT) != MODER_DEFAULT) {
        TEST_FAIL("Default Registers",
                  "MODER = 0x%08x, expected bits 0x%08x set",
                  moder, MODER_DEFAULT);
        ok = 0;
    }
    ...
    if (ipgt != IPGT_DEFAULT) {
        TEST_FAIL("Default Registers",
                  "IPGT = 0x%08x, expected 0x%08x", ipgt, IPGT_DEFAULT);
        ok = 0;
    }
    ...
    if (int_mask != INT_MASK_ALL) {
        TEST_FAIL("Default Registers",
                  "INT_MASK = 0x%08x, expected 0x%08x", int_mask, INT_MASK_ALL);
        ok = 0;
    }
    ...
    if (tx_bd_num == 0 || tx_bd_num > 0x80) {
        TEST_FAIL("Default Registers",
                  "TX_BD_NUM = %u, expected 1..128", tx_bd_num);
        ok = 0;
    }
...
static int test_mdio_access(void) {
    ...
    if (ioctl(fd, SIOCGMIIPHY, &ifr) < 0) {
        if (errno == EINVAL) {
            ...
            TEST_FAIL("MII/MDIO Access", "SIOCGMIIPHY failed: %s",
                      strerror(errno));
        }
        ok = 0;
    } else {
...
static int test_tx_basic(void) {
    ...
    if (ok) {
        ...
        if (stats_after.tx_errors > stats_before.tx_errors) {
            TEST_FAIL("TX Basic",
                      "tx_errors increased: before=%lu, after=%lu",
...
static int test_tx_sizes(void) {
    ...
    if (ok) {
        ...
        if ((long)tx_delta < num_tests) {
            TEST_FAIL("TX Size Range",
                      "Expected %d TX packets, got %lu", num_tests, tx_delta);
            ok = 0;
        }
        if (stats_after.tx_errors > stats_before.tx_errors) {
...
static int test_rx_basic(void) {
    ...
    if (ok) {
        if (recv_cmd(fd, &pkt, 5000) < 0) {
            ...
            if (plen != sizeof(data)) {
                TEST_FAIL("RX Basic", "Response payload length %u != %zu",
                          plen, sizeof(data));
                ok = 0;
    ...
    for (int i = 0; i < num_tests; i++) {
        ...
        if (recv_cmd(fd, &pkt, 5000) < 0) {
            ...
            if (plen != (uint16_t)sizes[i]) {
                TEST_FAIL("RX Size Range",
                          "Size %d: response length %u != %d", sizes[i], plen, sizes[i]);
                ok = 0;
...
static int test_loopback(void) {
    ...
    if (set_iface_flags(fd, IFF_LOOPBACK, 0) < 0) {
        TEST_INFO("Loopback Mode",
                  "IFF_LOOPBACK not supported by driver (%s)",
                  strerror(errno));
        close(fd);
    ...
    if (ok) {
        ...
        (void)flags;
        TEST_INFO("Loopback Mode",
                  "TX delta=%lu, RX delta=%lu (MODER_LOOP active)",
...
static int test_mtu_reject(void) {
    ...
    if (ioctl(fd, SIOCSIFMTU, &ifr) == 0) {
        ...
        ok = 0;
               ...
               errno != ENOSYS) {
        TEST_FAIL("MTU Rejection",
                  "SIOCSIFMTU failed with unexpected errno %d (%s)",
...
static int test_ring_params(void) {
    ...
    if (get_ethtool_ring(fd, &ring) < 0) {
        TEST_FAIL("Ring Parameters",
                  "ethtool get_ringparam failed: %s", strerror(errno));
        close(fd);
        return -1;
    }
    close(fd);
    ...
    if (g_verbose) {
        printf("    rx_max_pending=%u tx_max_pending=%u\n",
               ring.rx_max_pending, ring.tx_max_pending);
    ...
    if (ring.tx_pending > 0 && (ring.tx_pending & (ring.tx_pending - 1)) != 0) {
        TEST_FAIL("Ring Parameters",
                  "tx_pending = %u is not a power of 2", ring.tx_pending);
        ok = 0;
    }
    ...
    if (ring.rx_mini_pending != 0) {
        TEST_FAIL("Ring Parameters",
                  "rx_mini_pending = %u, expected 0", ring.rx_mini_pending);
        ok = 0;
    }
    if (ring.rx_jumbo_pending != 0) {
        TEST_FAIL("Ring Parameters",
                  "rx_jumbo_pending = %u, expected 0", ring.rx_jumbo_pending);
        ok = 0;
    }
...
static int test_register_dump(void) {
    ...
    if (get_ethtool_regs(fd, regs, &count) < 0) {
        ...
        TEST_FAIL("Register Dump",
                  "ethtool get_regs failed: %s", strerror(errno));
        close(fd);
        return -1;
    }
    close(fd);
    ...
    if (count < REG_ETH_END / 4) {
        TEST_FAIL("Register Dump",
                  "Only %d registers, expected >= %d", count, REG_ETH_END / 4);
        ok = 0;
    }
    ...
    static const char *reg_names[] = {
        "MODER", "INT_SOURCE", "INT_MASK", "IPGT",
        "IPGR1", "IPGR2", "PACKETLEN", "COLLCONF",
        "TX_BD_NUM", "CTRLMODER", "MIIMODER", "MIICOMMAND",
        "MIIADDRESS", "MIITX_DATA", "MIIRX_DATA", "MIISTATUS",
        "MAC_ADDR0", "MAC_ADDR1", "ETH_HASH0", "ETH_HASH1",
        "ETH_TXCTRL"
    };
...
static int test_interrupts(void) {
    ...
    if (irq_after <= irq_before) {
        TEST_FAIL("Interrupts",
                  "IRQ count did not increase: before=%lu, after=%lu",
...
static int test_stress_tx(void) {
    ...
    if (ok) {
        ...
        if ((long)tx_delta < count) {
            TEST_FAIL("Stress TX",
                      "tx_packets delta %lu < %d", tx_delta, count);
            ok = 0;
        }
        if (stats_after.tx_errors > stats_before.tx_errors) {
...
static int test_stress_rx(void) {
    ...
    if (ok) {
        ...
        if (received < count) {
            TEST_FAIL("Stress RX",
                      "Only received %d/%d frames", received, count);
            ok = 0;
        }
        if (stats_after.rx_errors > stats_before.rx_errors) {
...
static void print_usage(const char *prog) {
    ...
    fprintf(stderr, "  -i <iface>      Network interface (auto-detect if omitted)\n");
