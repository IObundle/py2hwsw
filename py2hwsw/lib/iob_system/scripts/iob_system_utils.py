...
def assert_block_attributes(attributes_dict, py_params):
    ...
    for attribute in ["subblocks", "superblocks"]:
        for block in attributes_dict.get(attribute, []) + child_attributes.get(
            ...
            assert block.get("core", None), f"All {attribute} must have a core name!"
            assert block.get(
                "instance_name", None
            ), f"Block '{block['core']}' must have an instance name!"
...
def generate_makefile_segments(attributes_dict, peripherals, params, py_params):
    ...
    with open(f"{build_dir}/software/auto_sw_build.mk", "w") as file:
        ...
        # NOTE: Ideally, the peripheral_name_list should contain the generated 'name' of peripherals (like iob_cache_axi) instead of the original 'core' of the peripheral (like iob_cache).
        peripheral_name_list = []
        for peripheral in peripherals:
            if peripheral["core"] not in peripheral_name_list:
                peripheral_name_list.append(peripheral["core"])
        if peripherals:
