...
- Structure:
    - `drivers/`: directory with linux kernel module drivers for {peripheral['name']}
        ...
        - `{peripheral['name']}_sysfs_files.h` and `{peripheral['name']}_sysfs.h`: header files
...
def create_driver_sysfs_header_file_list(path, peripheral):
    ...
    fswhdr.write(
        " * `create_driver_sysfs_header_file_list` method of `create_peripheral_device_drivers.py`\n"
    )
    fswhdr.write(" */\n\n")
    ...
    fswhdr.write(f"#ifndef H_{core_prefix}_SYSFS_FILES_H\n")
    fswhdr.write(f"#define H_{core_prefix}_SYSFS_FILES_H\n\n")
    ...
    fswhdr.write(f"\n#endif // H_{core_prefix}_SYSFS_FILES_H\n")

    fswhdr.close()


def create_driver_sysfs_header_file(path, peripheral, multi=False):
    ...
    fswhdr.write("/* This file was automatically generated by:\n")
    fswhdr.write(
        " * `create_driver_sysfs_header_file` method of `create_peripheral_device_drivers.py`\n"
    )
    fswhdr.write(" */\n\n")
...
#include "{peripheral['name']}_sysfs_files.h"
...
"""
    ...
    with open(os.path.join(path, f"{peripheral['name']}_csrs.c"), "w") as f:
        f.write(content)
...
HDR += ../drivers/{peripheral['name']}_sysfs_files.h
...
def generate_device_drivers(output_dir, peripheral):
    ...
    create_driver_sysfs_header_file_list(
        os.path.join(output_dir, "drivers"), _peripheral
    )
    ...
    create_user_csrs_source(os.path.join(output_dir, "user"), _peripheral)
