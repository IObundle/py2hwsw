...
- Structure:
    - `drivers/`: directory with linux kernel module drivers for {peripheral['name']}
        ...
        - `{peripheral['name']}.h` and `{peripheral['name']}_sysfs.h`: header files
...
def filter_csrs_list(csrs_list):
    filtered_csrs_list = [i for i in csrs_list if i["log2n_items"] == 0]
...
def create_driver_header_file(path, peripheral):
    """Generate <peripheral_name>.h file"""
    fswhdr = open(os.path.join(path, f"{peripheral['name']}.h"), "w")

    fswhdr.write("/* This file was automatically generated by:\n")
    fswhdr.write(" * `create_driver_header_file` method of `create_peripheral_device_drivers.py`\n")
    fswhdr.write(" */\n\n")
    ...
    fswhdr.write(f"#define {core_prefix}_CSRS_ADDR_W {peripheral['csrs_addr_w']}\n\n")

    fswhdr.write("//Addresses\n")
    for csr in peripheral["csrs"]:
        CSR_NAME = csr["name"].upper()
        if "W" in csr["mode"] or "R" in csr["mode"]:
            fswhdr.write(f"#define {core_prefix}_{CSR_NAME}_ADDR {csr['addr']}\n")
    ...
    for csr in peripheral["csrs"]:
        ...
        if n_bytes == 3:
            n_bytes = 4
        if "W" in csr["mode"] or "R" in csr["mode"]:
            fswhdr.write(f"#define {core_prefix}_{CSR_NAME}_W {n_bytes*8}\n")
...
def create_driver_sysfs_header_file(path, peripheral, multi=False): # (table, out_dir, top, multi=False):
    """Generate <peripheral_name>_sysfs.h file"""
    ...
    fswhdr.write("/* This file was automatically generated by:\n")
    fswhdr.write(" * `create_driver_sysfs_header_file` method of `create_peripheral_device_drivers.py`\n")
    fswhdr.write(" */\n\n")
    ...
    for csr in csrs_list:
        fswhdr.write(
            f"\tret |= device_create_file(device, &dev_attr_{csr['name']});\n"
        )
...
def generate_device_drivers(output_dir, peripheral):
    ...
    _peripheral = {
        ...
        "csrs": csrs_list,
        "csrs_addr_w": 4, # FIXME: This should be auto-determined
    }
