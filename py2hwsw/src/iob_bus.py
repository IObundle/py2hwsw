...
class iobInterface(iob_bus):
    ...
    def get_wires(self):
        ...
        wires = [
            ...
            iob_wire(name="iob_rvalid_i", descr="Read data valid."),
            iob_wire(name="iob_rdata_i", width=self.data_w, descr="Read data."),
            iob_wire(name="iob_ready_i", descr="Interface ready."),
        ]
...
class symMemInterface(_memInterface):
    ...
    def get_wires(self):
        ...
        match self.genre:
            case "rom_2p":
                self._set_mem_wires("", has_addr=False, has_enable=False)
                self.__set_mem_read_wires(
            ...
            case "rom_sp":
                ...
                self.__set_mem_read_wires("")
            case "rom_tdp":
                self._set_mem_wires("", has_enable=False, has_addr=False)
                self.__set_mem_read_wires(
            ...
            case "rom_atdp":
                self._is_async = True
                self._set_mem_wires("a")
                self.__set_mem_read_wires("a", is_true_port=True)
                self._set_mem_wires("b")
                self.__set_mem_read_wires("b", is_true_port=True)
            case "ram_2p":
                self._set_mem_wires("", has_addr=False, has_enable=False)
                self.__set_mem_read_wires(
                    "", has_enable=True, has_ready=True, has_addr=True
                )
                self.__set_mem_write_wires("", has_ready=True, has_addr=True)
            case "ram_at2p":
                self._is_async = True
                self._set_mem_wires("r", has_addr=False, has_enable=False)
                self.__set_mem_read_wires("", has_enable=True, has_addr=True)
                self._set_mem_wires("w", has_addr=False, has_enable=False)
                self.__set_mem_write_wires("", has_addr=True)
            case "ram_atdp":
                self._is_async = True
                self._set_mem_wires("a")
                self.__set_mem_read_wires("a", is_true_port=True)
                self.__set_mem_write_wires("a", is_true_port=True)
                self._set_mem_wires("b")
                self.__set_mem_read_wires("b", is_true_port=True)
                self.__set_mem_write_wires("b", is_true_port=True)
            case "ram_atdp_be":
                self._is_async = True
                self._set_mem_wires("a")
                self.__set_mem_read_wires("a", is_true_port=True)
                self.__set_mem_write_wires("a", is_true_port=True, has_byte_enable=True)
                self._set_mem_wires("b")
                self.__set_mem_read_wires("b", is_true_port=True)
                self.__set_mem_write_wires("b", is_true_port=True, has_byte_enable=True)
            case "ram_sp" | "ram_sp_se":
                self._set_mem_wires("")
                self.__set_mem_read_wires("")
                self.__set_mem_write_wires("")
            case "ram_sp_be":
                self._set_mem_wires("")
                self.__set_mem_read_wires("")
                self.__set_mem_write_wires("", has_byte_enable=True)
            case "ram_t2p":
                self._set_mem_wires("", has_addr=False, has_enable=False)
                self.__set_mem_read_wires("", has_enable=True, has_addr=True)
                self.__set_mem_write_wires("", has_addr=True)
            case "ram_t2p_be":
                self._set_mem_wires("", has_addr=False, has_enable=False)
                self.__set_mem_read_wires("", has_enable=True, has_addr=True)
                self.__set_mem_write_wires("", has_addr=True, has_byte_enable=True)
            case "ram_t2p_tiled":
                self._set_mem_wires("", has_enable=False)
                self.__set_mem_read_wires("", has_enable=True)
                self.__set_mem_write_wires("")
            case "ram_tdp":
                self._set_mem_wires("a")
                self.__set_mem_read_wires("a")
                self.__set_mem_write_wires("a")
                self._set_mem_wires("b")
                self.__set_mem_read_wires("b")
                self.__set_mem_write_wires("b")
            case "ram_tdp_be" | "ram_tdp_be_xil":
                self._set_mem_wires("a")
                self.__set_mem_read_wires("a")
                self.__set_mem_write_wires("a", has_byte_enable=True)
                self._set_mem_wires("b")
                self.__set_mem_read_wires("b")
                self.__set_mem_write_wires("b", has_byte_enable=True)
            case _:
                raise ValueError(f"Unknown memory interface genre: {self.genre}")

        self._remove_duplicate_wires()

    def __set_mem_read_wires(
        self,
        suffix: str = "",
    ...
    def __set_mem_write_wires(
        ...
        """Set write wires for the memory interface."""
...
class asymMemInterface(_memInterface):
    ...
    def get_wires(self):
        ...
        self._wires = self._remove_duplicate_wires(self._wires)
    ...
    def get_wires(self):
        ...
        if self.has_tlast:
            self._wires.append(
                iob_wire(
                    name="axis_tlast_o",
                    descr="AXI-Stream last wire output.",
                )
            )
...
class AXILiteInterface(iob_bus):
    ...
    def get_wires(self):
        ...
        if self.has_write_if:
            self.__set_write_wires()

    def __set_write_wires(self):
        """Set write wires for the AXI-Lite interface."""
        self._wires += [
            iob_wire(
                name="axil_awaddr_o",
                width=self.addr_w,
    ...
    def __set_read_wires(self):
        """Set read wires for the AXI-Lite interface."""
        self._wires += [
            iob_wire(
                name="axil_araddr_o",
                width=self.addr_w,
...
class AXIInterface(iob_bus):
    ...
    def __set_write_wires(self):
        """Set write wires for the AXI interface."""

        # AXI-Lite write wires
        self._wires += [
            iob_wire(
                name="axi_awaddr_o",
                width=self.addr_w,
    ...
    def __set_read_wires(self):
        """Set read wires for the AXI interface."""
...
class AHBInterface(iob_bus):
    ...
    def get_wires(self):
        ...
        self._wires += [
            ...
            iob_wire(
                name="ahb_write_o",
                descr="AHB write wire. Transfer direction: (1) Write; (0) Read.",
            ),
            iob_wire(
                name="ahb_rdata_i",
                width=self.data_w,
    ...
    def get_wires(self):
        ...
        if self.is_full:
            self._wires += [
                ...
                iob_wire(
                    name="wb_tga_o",
                    descr="Address tag type. Contains information associated with address lines [adr], and is qualified by wire [stb].",
                ),
                iob_wire(
                    name="wb_tgc_o",
                    descr="Cycle tag type. Contains information associated with bus cycles, and is qualified by wire [cyc].",
                ),
            ]
